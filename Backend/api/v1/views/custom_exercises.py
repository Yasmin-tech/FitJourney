...
def get_all_custom_exercises():
    ...
    return (
        jsonify([custom_exercise.to_dict() for custom_exercise in custom_exercises]),
        200,
    )


@views_bp.route(
    "/custom_exercises/title/<string:title>", methods=["GET"], strict_slashes=False
)
@jwt_required()
@user_exists
def get_custom_exercise_by_title(title):
    ...
    if (
        custom_exercise.user.email != log_in_user_email
...
def get_all_custom_exercise_categories():
    ...
    if not custom_exercises:
        return jsonify([]), 200
    categories = (
        db.session.query(CustomExercise.category)
        .filter(CustomExercise.user_id == user_id)
        .distinct()
        .all()
    )
    categories = [
...
@views_bp.route(
    "/custom_exercises/muscle_groups", methods=["GET"], strict_slashes=False
)
@jwt_required()
@user_exists
def get_all_custom_exercise_muscle_groups():
    ...
    if not custom_exercises:
        return jsonify([]), 200
    muscle_groups = (
        db.session.query(CustomExercise.muscle_group)
        .filter(CustomExercise.user_id == user_id)
        .distinct()
        .all()
    )
...
@views_bp.route(
    "/custom_exercises/muscle_groups/<string:muscle_group>",
    methods=["GET"],
...
def get_all_custom_exercises_by_muscle_group(muscle_group):
    ...
    if not custom_exercises:
        return jsonify([]), 200
    custom_exercises = (
        db.session.query(CustomExercise)
        .filter(
            CustomExercise.user_id == user_id,
            CustomExercise.muscle_group == muscle_group,
        )
        .all()
    )
...
@views_bp.route(
    "/custom_exercises/<int:custom_exercise_id>", methods=["GET"], strict_slashes=False
)
@roles_required("Admin", "Developer")
...
@views_bp.route(
    "/users/<int:user_id>/custom_exercises", methods=["GET"], strict_slashes=False
)
@jwt_required()
@user_exists
def get_user_custom_exercises(user_id):
    ...
    return (
        jsonify(
            [custom_exercise.to_dict() for custom_exercise in user.custom_exercises]
        ),
        200,
    )
...
@views_bp.route(
    "/users/<int:user_id>/custom_exercises/<int:custom_exercise_id>",
    methods=["GET"],
    strict_slashes=False,
)
@jwt_required()
...
def get_user_custom_exercise(user_id, custom_exercise_id):
    ...
    custom_exercise = next(
        (
            ...
            if custom_exercise.id == custom_exercise_id
        ),
        None,
    )
...
@views_bp.route(
    "/users/<int:user_id>/custom_exercises", methods=["POST"], strict_slashes=False
)
@jwt_required()
@user_exists
def create_exercise_fot_user(user_id):
    ...
    query = db.select(CustomExercise).where(
        CustomExercise.title == data["title"], CustomExercise.user_id == user_id
    )
...
def delete_custom_exercise(user_id, custom_exercise_id):
    ...
    if custom_exercise.media_file_url:
        try:
            result, message = drive.delete_file(
                webContentLink=custom_exercise.media_file_url
            )
...
@views_bp.route(
    "/users/<int:user_id>/custom_exercises/<int:custom_exercise_id>/media_file",
    methods=["GET"],
    strict_slashes=False,
)
@jwt_required()
...
@views_bp.route(
    "/users/<int:user_id>/custom_exercises/<int:custom_exercise_id>/upload_media",
    methods=["POST"],
    strict_slashes=False,
)
@jwt_required()
...
@views_bp.route(
    "/users/<int:user_id>/custom_exercises/<int:custom_exercise_id>/update_media",
    methods=["PUT"],
    strict_slashes=False,
)
@jwt_required()
...
@views_bp.route(
    "/users/<int:user_id>/custom_exercises/<int:custom_exercise_id>/delete_media",
    methods=["DELETE"],
    strict_slashes=False,
)
@jwt_required()
